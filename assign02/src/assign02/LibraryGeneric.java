...
public class LibraryGeneric<Type> {
	...
	ArrayList<LibraryBookGeneric<Type>> patronBooks = new ArrayList<>();
	for (LibraryBookGeneric<Type> libraryBook : library)
	    if (libraryBook.getPatron() != null && libraryBook.getPatron().equals(patron))
	...
	ArrayList<LibraryBookGeneric<Type>> bookList = this.lookup(patron);
	if (bookList.size() == 0)
	    return false;
	for (LibraryBookGeneric<Type> libraryBook : bookList)
	    libraryBook.checkIn();
	return true;
    }
     ...
     */
    private static <InnerType> void sort(List<LibraryBookGeneric<InnerType>> list,
	    Comparator<LibraryBookGeneric<InnerType>> cmp) {
    ...
    /**
     * Gets a list of books in this library sorted by ISBN, smallest to largest. If
     * the library is empty, returns an empty list.
     *
     * @return list of all library books sorted by ISBN
     */
    public List<LibraryBookGeneric<Type>> getListSortedByIsbn() {
	List<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<>();
	libraryCopy.addAll(this.library);

	OrderByIsbn<Type> comparator = new OrderByIsbn<>();
    ...
    /**
     * Gets a list of books in this library sorted by author surname,
     * lexicographically. For books with the same author surname, break ties using
     * book titles. If the library is empty, returns an empty list.
     *
	...
	OrderByAuthor<Type> comparator = new OrderByAuthor<>();
	sort(libraryCopy, comparator);
    ...
    /**
     * Gets the list of books in this library with a due date older than the input
     * date. The list is sorted by date, oldest to most recent. If no library books
     * are overdue, returns an empty list.
     *
	...
	OrderByDueDate<Type> comparator = new OrderByDueDate<>();
	List<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<>();
	libraryCopy.addAll(this.library);
	// Removes the books that are either checked in or not overdue.
	libraryCopy.removeIf(book -> (book.getDueDate() == null) || (book.getDueDate().compareTo(overDueDate) >= 0)); // We
														      // found
														      // removeIf
														      // from
														      // the
